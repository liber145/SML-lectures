...
$X_i \in \BR^p$, we want to find $Z_i \in \BR^q (p > q)$ to present $X_i$.
If we want to use linear transformation, then our purpose is to find a matrix $A$ so that $Z_i = AX_i$.
If we want to use nonlinear transformation, then we need to find a nonlinear function $f$ so that $f(X_i) = Z_i$.

\textbf{Clustering: }
We can view the n samples as n points, and cluster them to $k$ clusters, where $k$ is determined by us.

\textbf{Classification: }
We have a label $Y_i$ for each $X_i$. And $Y = (Y_1, Y_2, \cdots, Y_n)^T$. And $Y_i \in C$, where $C$ is a finite set.

\textbf{Regression: }
Like classification, but $Y_i \in \BR$.

\textbf{Ranking: }
Isotonic regression (IR) involves finding a weighted least-squares fit $x \in \BR^n $ to a vector $a \in \BR^n$ with weights vector $w \in \BR^n$ subject to a set of non-contradictory constraints of kind $x_i \geq x_j$.

\textbf{Multi-task: }
Like Classification, but $Y_i$ can have multi value.
