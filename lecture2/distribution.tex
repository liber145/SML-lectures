...
\newcommand{\ho}[5]{\notes{#1}{Distributions}{Professor: Zhihua Zhang}{}{Lecture Notes #1: Distributions}}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
The CDF of a continuous random variable X can be expressed as the integral of its probability density function(pdf) $f_X(x)$ as follows:$$F(x) = \int_{-\infty}^{x}f_X(t)dt$$ and $$F'(x) = f_X(x)$$
\begin{lemma}
...
\item
$\sum{f(x)}=1$ or $\int{f(x)}=1$ sometimes is written as $\int{dF(x)}=1$ or $\int{F(dx)}=1$. 
\item
We call $X$ and $Y$ are equal in distribution iff $F_{X}(x)=F_{Y}(x)$ for any $x$. Notice that it is \textbf{not} equal to $X=Y$. For example,  $Pr(X=1)=Pr(X=-1)=\frac{1}{2}$. Let $Y=-X$, then $X$ and $Y$ are equal in distribution but $X\neq Y$.
\end{enumerate}
...
\subsubsection{Disac Distribution}
